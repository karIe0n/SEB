...
daily_rates = pd.read_csv("https://www.ecb.europa.eu/stats/eurofxref/eurofxref.zip", compression="zip")
historical_rates = pd.read_csv("https://www.ecb.europa.eu/stats/eurofxref/eurofxref-hist.zip", compression="zip")

def mean_exchange_rate(rates_list):
...
df = pd.DataFrame(data, index=["Today's Rate", "Average Rate"]).T  # transpose
html = df.to_html(border=1)

with open("exchange_rates.html", "w") as file:
    file.write(html)
    
file_path = os.path.abspath("exchange_rates.html")
